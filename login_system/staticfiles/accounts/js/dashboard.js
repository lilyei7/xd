...
async function loadSucursales() {
    ...
    try {
        ...
        // Verificar si es gerente para usar un endpoint específico
        if (window.userPermissions && window.userPermissions.gerente && !window.userPermissions.admin && !window.userPermissions.superuser) {
            console.log("🔒 Usuario con rol gerente detectado, usando endpoint específico");
            endpoint = '/sucursales/gerente/';
        }
        
        console.log(`🔄 Cargando sucursales desde: ${endpoint}`);
        
        const response = await fetch(endpoint, {
            headers: {
                'X-Requested-With': 'XMLHttpRequest',  // Indicar que es una solicitud AJAX
                'Accept': 'application/json'           // Solicitar específicamente JSON
            },
            credentials: 'same-origin'                 // Enviar cookies para autenticación
        });
        
        // Verificar si la respuesta es JSON antes de intentar parsearla
        const contentType = response.headers.get('content-type');
        if (!contentType || !contentType.includes('application/json')) {
            // Si no es JSON, podría ser HTML de página de login o error
            console.error("❌ La respuesta no es JSON:", contentType);
            
            if (response.status === 401 || response.status === 403) {
                grid.innerHTML = `
                    <div class="error-message">
                        <h3>Sesión expirada o sin permisos</h3>
            ...
            grid.innerHTML = `
                <div class="error-message">
                    ...
                    <p>No se pudo obtener la lista de sucursales.</p>
                    <button onclick="loadSucursales()">Intentar nuevamente</button>
                </div>
            `;
            return;
        }
        
        // Ahora que sabemos que es JSON, podemos procesarlo
        ...
        if (data.status === 'success') {
            if (!data.sucursales || !data.sucursales.length) {
                grid.innerHTML = `
                    <div class="empty-state">
                        ...
                        <p>No hay sucursales disponibles para mostrar.</p>
                    </div>
                `;
                return;
            }
            ...
            grid.innerHTML = data.sucursales.map(sucursal => `
                <div class="sucursal-card">
                    ...
                    <div class="sucursal-actions">
                        ...
                        ${window.userPermissions && (window.userPermissions.admin || window.userPermissions.superuser) ? 
            ...
            throw new Error(data.message || 'Error desconocido al cargar sucursales');
        }
    } catch (error) {
        ...
        grid.innerHTML = `
            <div class="error-message">
                <h3>Error al cargar sucursales</h3>
                <p>${error.message || 'No se pudo obtener la lista de sucursales.'}</p>
                <button onclick="loadSucursales()">Intentar nuevamente</button>
            </div>
        `;
    }
